...
configureNavigation({ renderStep, renderFinal });
...
function initializeApp() {
  ...
  try {
    ...
    renderRuntimeError(error, 'Não foi possível iniciar o quiz.');
  }
}
...
function assertRequiredElements() {
  if (!root || !header || !backButton || !progressBar || !progressLabel) {
    throw new Error('Estrutura HTML obrigatória do quiz não encontrada.');
  }
}
...
export function renderStep(stepId) {
  try {
    ...
    if (step.type === 'hero') {
      ...
      if (!startButton) throw new Error('Botão inicial não encontrado.');
...
export function renderFinal(finalStatus, options = {}) {
  try {
    ...
    renderRuntimeError(error, 'Não foi possível exibir a etapa final.');
  }
}

function renderRuntimeError(error, userMessage) {
  if (fatalErrorVisible || !root) return;
  ...
  root.innerHTML = `
    <section class="runtime-error">
      ...
      <h2>Vamos recarregar esta etapa</h2>
      <p>${escapeHtml(userMessage)}</p>
      <button class="cta-primary" id="reload-quiz-button" type="button">Recarregar quiz</button>
    </section>
  `;
